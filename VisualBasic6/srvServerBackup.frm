...
Begin VB.Form frmLunchServer 
   ...
   Begin MSDBGrid.DBGrid DBGrid1 
      Bindings        =   "srvServerBackup.frx":031F
      Height          =   2535
      Left            =   120
      OleObjectBlob   =   "srvServerBackup.frx":0331
      TabIndex        =   5
      Top             =   120
      Width           =   7095
   End
...
Private Sub LunchTimer_Timer()
    ...
    If Format(timNextTime, "hh:mm:ss AMPM") = timNOW Then
        ...
        If rstgroups.RecordCount > 0 Then rstgroups.MoveFirst
...
Private Sub Winsock1_DataArrival(ByVal bytesTotal As Long)
  ...
  If Right(strGetData, 2) = vbCrLf Then strGetData = Left(strGetData, Len(strGetData) - 2)
    ...
    count = GetParams(strGetData, strParm(), 2)

  Let strRequest = strParm(0)

  Select Case strParm(0)
  Case Is = "ADU"   'Add User

  Case Is = "DLU"   'Delete User
  
  Case Is = "JTG"   'Joing This Group
        count = GetParams(strParm(1), strParm(), 2)
        Let strQuery = "Select username from users where username = '" & strParm(0) & "'"
        Let success = SubmitQuery(strQuery, strSendData)
        If (success > 0) Then
            Let strQuery = "update users set groupid = '" & strParm(1) & "' where username = '" & strParm(0) & "'"
            Let strError = "ERR Error updating user group"
            thissuccess = SubmitQuery(strQuery, strSendData)
            Let strQuery = "Select groupid from users where username = '" & strParm(0) & "'"
        Else
            Let strQuery = ""
  ...
  Case Is = "UNT"   'Update Notification Time for group
        ...
        If (success > 0) Then
            ...
            Let strQuery = "update groups set notificationtime = #" & Format(strParm(1), "hh:mm:ss AMPM") & "# where groupid = '" & strGroupID & "'"
            Let strError = "ERR Error updating user group"
            thissuccess = SubmitQuery(strQuery, strSendData)
            If thissuccess = 0 Then
                Let strQuery = "update groups set notificationtime = #12:00:00 PM# where groupid = '" & strParm(1) & "'"
                thissuccess = SubmitQuery(strQuery, strSendData)
            End If
  ...
  Case Is = "URW"   ' Update Restaurant Weight
        count = GetParams(strParm(1), strParm(), 3)
        Let strQuery = "Select groupid from users where username = '" & strParm(0) & "'"
        Let success = SubmitQuery(strQuery, strSendData)
        If (success > 0) Then
            Let strGroupID = Mid(strSendData, InStr(strSendData, vbCrLf) + 2)
            Let strGroupID = Left(strGroupID, InStr(strGroupID, vbTab) - 1)
            Let strQuery = "update restaurants set grouprating = '" & strParm(1) & "' where groupid = '" & strGroupID & "'" & " and restaurantname = '" & strParm(2) & "'"
            Let strError = "ERR Error updating user group"
            thissuccess = SubmitQuery(strQuery, strSendData)
            Let strQuery = "Select grouprating from restaurants where groupid = '" & strGroupID & "' and restaurantname = '" & strParm(2) & "'"
        Else
            Let strQuery = ""
            Let strError = "ERR UserName not found"
        End If
  Case Is = "ADG"   'Add Group
    count = GetParams(strParm(1), strParm(), 2)
    strQuery = "Insert into groups values('" & strParm(1) & "', #12:00:00 PM#,' ',0,0, FALSE,#12:10:00 PM#)"
    Let success = SubmitQuery(strQuery, strSendData)
    strQuery = "update users set groupid = '" & strParm(1) & "' where username = '" & strParm(0) & "'"
    Let success = SubmitQuery(strQuery, strSendData)
    Let strQuery = "Select Groupid from users where username = '" + strParm(0) + "'"
    Let strError = "ERR Error Adding Group" & strParm(1)
  ...
  Case Is = "GCG"   'Get This Users Group
    Let strQuery = "Select Groupid from users where username = '" + strParm(1) + "'"
  ...
  Case Is = "GRR"   'Get Random Restaurant
        strRating = GetRandomRating(strParm(1), Ignore)
        If Left(strRating, 3) = "ERR" Then
            Let strRestaurantName = Mid(strRating, 5)
        Else
            Set qryTemp = dtUsers.Database.CreateQueryDef("", _
              "Select restaurantname from restaurants where groupid = '" _
               & strParm(1) & "' and grouprating = '" & strRating & "'")
            Set rstChoice = qryTemp.OpenRecordset
            rstChoice.MoveLast
            rstChoice.MoveFirst
  ...
  Case Is = "ARG"   'Add Restaurant to group (username,restaurant name)
        count = GetParams(strParm(1), strParm(), 2)
        Let strQuery = "Select groupid from users where username = '" & strParm(0) & "'"
        Let success = SubmitQuery(strQuery, strSendData)
        If (success > 0) Then
            Let strGroupID = Mid(strSendData, InStr(strSendData, vbCrLf) + 2)
            Let strGroupID = Left(strGroupID, InStr(strGroupID, vbTab) - 1)
            Let strQuery = "select restaurantname from restaurants where restaurantname = '" & strParm(1) & "' and groupid in ('" & strGroupID & "','All')"
            Let strError = "ERR Error Restaurant already exists"
            thissuccess = SubmitQuery(strQuery, strSendData)
            If thissuccess = 0 Then
                Let strQuery = "insert into restaurants values('" & strGroupID & "','" & strParm(1) & "','Often',FALSE)"
                Let strError = "ERR Error updating user group"
                thissuccess = SubmitQuery(strQuery, strSendData)
                If UCase(strGroupID) <> "All" Then
                    Let strQuery = "insert into restaurants values('All','" & strParm(1) & "','Often',FALSE)"
                    thissuccess = SubmitQuery(strQuery, strSendData)
                End If
                Let strQuery = "Select distinct restaurantname from restaurants where restaurantname = '" & strParm(1) & "' and groupid = '" & strGroupID & "'"
            ElseIf thissuccess = 1 Then
                Let strQuery = "insert into restaurants values('" & strGroupID & "','" & strParm(1) & "','Often',FALSE)"
                Let strError = "ERR Error updating user group"
                thissuccess = SubmitQuery(strQuery, strSendData)
                Let strQuery = "Select distinct restaurantname from restaurants where restaurantname = '" & strParm(1) & "' and groupid = '" & strGroupID & "'"
  ...
  Case Is = "DRG"   'Delete Restaurant
        ...
        If (success > 0) Then
            Let strGroupID = Mid(strSendData, InStr(strSendData, vbCrLf) + 2)
            Let strGroupID = Left(strGroupID, InStr(strGroupID, vbTab) - 1)
            Let strQuery = "delete from restaurants where groupid = '" & strGroupID & "' and restaurantname = '" & strParm(1) & "'"
            Let strError = "ERR Error updating user group"
        Else
            Let strQuery = ""
            Let strError = "ERR UserName not found"
  ...
  Case Is = "GGR"   'Get Group Restaurant Information
        Let strQuery = "Select restaurantname, grouprating from restaurants where groupid = '" + strParm(1) + "' order by restaurantname"
        Let strError = "ERR No restaurants for group " + strParm(1)
  Case Is = "SQL"   'SQL query
        Let strQuery = strParm(1)
        Let strError = "ERR SQL Error"
  Case Is = "LGI"   'Log in
        Dim LoggedOn As Boolean
        Dim IPAddress As String
        
        If UCase(Left(strParm(1), 4)) = "ADD " Then strAdd = True: strParm(1) = Mid(strParm(1), 5)
        count = GetParams(strParm(1), strParm())

        If strParm(1) = frmLunchClient.Winsock2.LocalIP Then
            Let strQuery = "select u.groupid, g.confirmed, g.notificationtime, g.currentrestaurantchoice " & _
                       "from users u, groups g " & _
                       "where u.groupid = g.groupid " & _
                       "and u.username = '" & strParm(0) & "'"
            Set qryTemp = dtUsers.Database.CreateQueryDef("", strQuery)
            Set rstChoice = qryTemp.OpenRecordset
            success = rstChoice.RecordCount
            LoggedOn = False
        Else
            Let strQuery = "select u.groupid, g.confirmed, g.notificationtime, g.currentrestaurantchoice, u.loggedon, u.ipaddress " & _
                       "from users u, groups g " & _
                       "where u.groupid = g.groupid " & _
                       "and u.username = '" & strParm(0) & "'"
            Set qryTemp = dtUsers.Database.CreateQueryDef("", strQuery)
            ...
            success = rstChoice.RecordCount
            If success > 0 Then
                LoggedOn = rstChoice.Fields("loggedon").Value
                IPAddress = rstChoice.Fields("ipaddress").Value
            End If
        End If

            
        If ((success > 0) And (strAdd = False) And Not LoggedOn) Or _
            ((success > 0) And (strAdd = False) And (IPAddress = strParm(1))) Then
            Let strPleaseHold = strQuery
            Let strQuery = "Update users set IPAddress = '" & strParm(1) & "', port = '" & strParm(2) & "', loggedon = true where username = '" & strParm(0) & "'"
            Let strError = "ERR Error updating user info"
            thissuccess = SubmitQuery(strQuery, strSendData)
            Let strQuery = strPleaseHold
        ElseIf (success > 0) And LoggedOn = True Then
            Let strQuery = ""
            Let strSendData = "ERR UserName Already Logged On"
        ElseIf (success > 0) And (strAdd = True) Then
            Let strQuery = ""
            Let strSendData = "ERR UserID already exists!"
        ElseIf (success = 0) And (strAdd = True) Then
            Let strQuery = "insert into users values('" & strParm(0) & "','All','" & strParm(1) & "','" & strParm(2) & "',true)"
            Let addsuccess = SubmitQuery(strQuery, strSendData)
        Let strQuery = "select u.groupid, g.confirmed, g.notificationtime, g.currentrestaurantchoice " & _
                       "from users u, groups g " & _
                       "where u.groupid = g.groupid " & _
                       "and u.username = '" & strParm(0) & "'"
        Let strError = "ERR Error adding " & strParm(0)
        ElseIf (success = 0) And (strAdd = False) Then
            Let strQuery = ""
            Let strSendData = "ERR No Users Found"
        End If
  Case Is = "LGO"
        strQuery = "update users set loggedon = FALSE where username = '" & strParm(1) & "'"
  Case Is = "SYV"
        Let strQuery = "update groups set currentyesvotes = (currentyesvotes + 1) where groupid = '" & strParm(1) & "'"
  Case Is = "SNV"
        Let strQuery = "update groups set currentnovotes = (currentnovotes + 1) where groupid = '" & strParm(1) & "'"
  Case Is = "BYE"   'close socket'
        strSendData = "C-Ya!" & vbCrLf & vbCrLf
        strControl = "BYE"
  Case Is = "HLP"
        strQuery = ""
        strSendData = GetHelpScreen(strParm(1))
  Case Else
